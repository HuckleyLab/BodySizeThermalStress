#Alanysis for a NCC news and views
...
temp=clim$TempC
#potential sources to update cliamte data:
...
bs= read.csv("TsengDryad.csv")
bs= subset(bs,bs$species=="angusticollis") 
bl= 1.56*bs$Elytron.Length 
dm= 0.03069*bl^2.63885
bs$mass= dm/0.4

plot(bs$Year, bs$mass)

mod1=lm(bs$mass~bs$Year)
#1037-0.424*yr

#1920: 1037-0.424*1920= 222.9mg= 0.22
#2020: 1037-0.424*2020= 180.52mg= 0.18
...
#-----------------
#B. mass specific, CHANGE TO TIME AS X AXIS
a <- 2
b <- 0.75
Q10 <- 2.5
M <- 10^c(-6:3)
...
plot(log10(hrs), tsm.l, type="l", ylab="thermal safety margins (C)", xlab="log10 time", 
